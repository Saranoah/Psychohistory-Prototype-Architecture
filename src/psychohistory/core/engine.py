...
try:
    ...
    class CivilizationMetrics:
        ...
        def get_metric(self, category, metric):
            return self.current_state.get(category, {}).get(metric, 0.0)
...
class PsychohistoryEngine:
    ...
    def _run_analysis_logic(self, metrics: CivilizationMetrics, analysis_date: datetime) -> AnalysisResult:
        ...
        normalized = self._normalize_metrics(current_data['metrics'], current_data.get('trends', {}))
        ...
        for category, metric_dict in current_state.items():
            for metric_name, current_value in metric_dict.items():
                ...
                if 'growth' in metric_name.lower() or 'adoption' in metric_name.lower():
                    ...
                    # Linear projection for stable metrics
                ...
                projected_metrics.update_metric(
                    MetricCategory[category.upper()], 
                    metric_name, 
                    projected_value
                )
        
        # Take snapshot of projected state
    ...
    def simulate_intervention(self, civ_name: str, intervention: Dict[str, float], 
        ...
        # Calculate success probability based on intervention magnitude and current risk
    ...
    def _apply_intervention_effects(self, metrics: CivilizationMetrics, 
        ...
        for intervention_key, effect in intervention.items():
            ...
            try:
                ...
                # Apply side effects to related metrics
    ...
    def _apply_interventions(self, metrics: CivilizationMetrics, 
        ...
        for intervention in interventions:
            # Interventions might have delayed effects
            start_year = intervention.get('start_year', 0)
            if years_elapsed >= start_year:
                # Apply with possible decay over time
    ...
    def _calculate_success_probability(self, intervention: Dict, current_risk: float) -> float:
        ...
        risk_factor = 1.0 - (current_risk * 0.5)  # Higher risk = lower success probability
        
        base_prob = min(0.9, total_magnitude * 2.0)  # Cap at 90%
        return max(0.1, base_prob * risk_factor)  # Minimum 10% chance
    
    def _calculate_cost_benefit(self, intervention: Dict, effectiveness: float) -> float:
        """Calculate cost-benefit ratio of intervention"""
        total_cost = sum(abs(v) for v in intervention.values())  # Simplified cost measure
        if total_cost == 0:
    ...
    def _generate_year_predictions(self, analysis: AnalysisResult, year: int) -> List[Dict]:
        ...
        for match in analysis.pattern_matches[:3]:
            confidence_decay = max(0.1, 1.0 - (year * 0.08))  # 8% decay per year
            predictions.append({
                'type': 'pattern_based',
    ...
    def _predict_key_events(self, analysis: AnalysisResult, year: int) -> List[Dict]:
        """Predict key events based on current patterns and metrics"""
        events = []
        ...
        if economic_stress > 0.7:
            events.append({
                ...
                'description': 'Potential financial instability or recession',
                'confidence': economic_stress * 0.8,
                'year': year
            })
        
        # Political events
        trust_level = analysis.metric_snapshot.get('POLITICAL_institutional_trust', 0.5)
        if trust_level < 0.3:
            events.append({
                'type': 'political_instability',
                'description': 'Risk of governance challenges or leadership crises',
                'confidence': (1.0 - trust_level) * 0.7,
                'year': year
            })
    ...
    def _calculate_pattern_match(self, metrics: Dict, pattern: HistoricalPattern) -> float:
        """Calculate pattern match with temporal consideration"""
        total_weight = 0.0
        matched_weight = 0.0
    ...
    def _calculate_composite_risk(self, metrics: Dict, pattern_matches: List[Dict]) -> float:
        ...
        # Trend momentum risk
        trend_risk = 0.0
        negative_trends = [t for t in metrics['trends'].values() if t < -0.1]  # Worsening
        if negative_trends:
    ...
    def _generate_recommendations(self, metrics: Dict, pattern_matches: List[Dict]) -> List[Dict]:
        """Generate actionable recommendations (simplified)"""
        # Implementation would load from external config
        return []
...
def main():
    ...
    # Create sample civilization metrics
    ...
    # Generate predictions
    print("\n2. Generating Timeline Predictions...")
    timeline = engine.predict_timeline("Test Civilization", years=3)
    
    for prediction in timeline:
        print(f"\n{prediction.timeframe}:")
        print(f"  Risk: {prediction.risk_score:.3f} ({prediction.risk_level})")
        print(f"  Confidence: {prediction.confidence:.1%}")
        for pred in prediction.predictions[:1]:  # Show first prediction
            print(f"  Prediction: {pred['outcome']}")
    
    # Simulate intervention
    print("\n3. Simulating Intervention...")
    intervention = {
        "ECONOMIC_wealth_inequality": -0.2,  # Reduce inequality
        "POLITICAL_institutional_trust": 0.15  # Increase trust
    }
    
    result = engine.simulate_intervention("Test Civilization", intervention, years=3)
    ...
    print(f"Cost-Benefit Ratio: {result.cost_benefit_ratio:.2f}")
